...
`)},async _copyFilteredComponentReference(){var n;const t=(((n=this.aiFilteredComponentsInput)==null?void 0:n.value)||"").trim();if(!t){m.toast("Pegá la lista de componentes que te dio la IA (separados por coma).","danger");return}const a=t.replace(/[\[\]"]/g,"").split(",").map(l=>l.trim().toLowerCase()).filter(l=>l.length>0);if(a.length===0){m.toast("No se encontraron nombres de componentes válidos.","danger");return}const s=this._buildFilteredComponentReference(a);if(await W.copyTextSmart(s,"Referencia Filtrada de Componentes")==="modal")return;this.aiStep3&&(this.aiStep3.style.display=""),this.aiStep2Instructions&&(this.aiStep2Instructions.style.display="");const o=this.btnCopyFilteredRef.innerHTML;this.btnCopyFilteredRef.innerHTML='<i class="fa-solid fa-check d-block mb-1 fs-5"></i> ¡Copiado!',this.btnCopyFilteredRef.classList.replace("btn-dark","btn-success"),setTimeout(()=>{document.body.contains(this.btnCopyFilteredRef)&&(this.btnCopyFilteredRef.innerHTML=o,this.btnCopyFilteredRef.classList.replace("btn-success","btn-dark"))},3e3)},_buildFromAi(){const t=this.aiJsonInput.value.trim();if(!t){m.toast("Pega el JSON que te devolvió la IA.","danger");return}let e;try{const s=t.replace(/```json/gi,"").replace(/```/g,"").trim(),i=s.indexOf("["),o=s.lastIndexOf("]"),n=i!==-1&&o!==-1&&o>i?s.slice(i,o+1):s;if(e=JSON.parse(n),!Array.isArray(e))throw new Error("El resultado no es un Array (Arreglo) válido.")}catch(s){m.toast("Error de lectura JSON: asegurate de pegar la respuesta completa de la IA (incluida la lista en lenguaje natural, si la trae). "+s.message,"danger");return}if(this._appendElementsToCanvas(e),this.aiJsonInput.value="",this.descInput&&!this.descInput.value.trim()&&this.aiIdeaInput){const s=this.aiIdeaInput.value.trim();s&&(this.descInput.value=s)}m.toast(`${e.length} elemento(s) agregado(s) a la Estructura del Documento`,"success");const a=document.getElementById("tab-btn-structure");a&&window.bootstrap&&window.bootstrap.Tab.getOrCreateInstance(a).show()},getAvailableVariables(t,e=null){const a=[];if(this.canvas.querySelectorAll(re).forEach(i=>{if(!i.closest(k.table)&&typeof i.getData=="function"){const o=i.getData();a.push({id:o.id,title:o.title||"(Sin Título)",type:"general"})}}),t==="table"&&e){const i=document.getElementById(e);if(i&&typeof i.getData=="function"){const o=i.getData();o.columns&&o.columns.forEach(n=>{a.push({id:n.id,title:`[Col] ${n.title}`,type:"column"})})}}return t==="general"&&this.canvas.querySelectorAll(k.table).forEach(o=>{const n=o.getData();n.columns&&n.columns.forEach(l=>{l.total&&a.push({id:`TOTAL_${o.id}_${l.id}`,title:`SUM: ${n.title} > ${l.title}`,type:"total"})})}),a},getApiVariables(t,e=null){const a=[];if(this.canvas.querySelectorAll(re).forEach(i=>{if(!i.closest(k.table)&&typeof i.getData=="function"){const o=i.getData();a.push({id:o.id,title:o.title||"(Sin Título)",type:"general"})}}),t==="table"&&e){const i=document.getElementById(e);if(i&&typeof i.getData=="function"){const o=i.getData();o.columns&&o.columns.forEach(n=>{a.push({id:n.id,title:`[Columna] ${n.title}`,type:"column"})})}}return a}},_e={init(){de.init({onNavigateToEditor:()=>this.showEditor(),onEditTemplate:t=>{this.showEditor(),z.load(t)}}),z.init({onBack:()=>this.showList(),onSaveSuccess:()=>{this.showList(),this.renderList()}}),this.renderList()},renderList(){de.render()},showEditor(){const t=document.getElementById("designer-list-view"),e=document.getElementById("designer-editor-view");t&&(t.style.display="none"),e&&(e.style.display="block"),z.reset()},showList(){const t=document.getElementById("designer-list-view"),e=document.getElementById("designer-editor-view");t&&(t.style.display="block"),e&&(e.style.display="none")},getAvailableVariables(t,e){return z.getAvailableVariables(t,e)},getApiVariables(t,e){return z.getApiVariables(t,e)}},J={handlers:{},currentDocs:[],currentTemplates:[],init(t){this.handlers=t;const e=document.getElementById("doc-list-view");if(!e){console.error("❌ [DocumentList] Error crítico: No se encontró #doc-list-view en el DOM.");return}e.innerHTML=this.getBaseHTML(),this.cacheDOM(),this.initTableEngine(),this.bindEvents()},getBaseHTML(){return`
    ...
    `},_bindListActions(t){t.querySelectorAll(".btn-view-doc").forEach(e=>{e.onclick=()=>this.handlers.onViewDocument(e.dataset.id)}),t.querySelectorAll(".btn-del-doc").forEach(e=>{e.onclick=async()=>{await m.confirm("Eliminar Documento","¿Estás seguro de eliminar este documento definitivamente? Esta acción no se puede deshacer.")&&(await b.delete("doc",e.dataset.id),m.toast("Documento eliminado correctamente","success"),this.render())}}),t.querySelectorAll(".btn-copy-id").forEach(e=>{e.onclick=async a=>{a.stopPropagation();const s=e.dataset.id,i=e.querySelector("i");try{await navigator.clipboard.writeText(s);const o=i.className;i.className="fa-solid fa-check text-success",setTimeout(()=>{i.className=o},1500)}catch(o){console.error("Error al copiar ID:",o),m.toast("No se pudo copiar el ID al portapapeles.","danger")}}}),t.querySelectorAll(".btn-dev-json").forEach(e=>{e.onclick=async a=>{a.stopPropagation();const s=e.dataset.id,i=e.querySelector("i"),o=i.className;try{i.className="fa-solid fa-spinner fa-spin text-info";let n=await b.getOne("doc",s);if(!n)throw new Error("Documento no encontrado");if(n.isEncrypted&&n.decryptionError){const u=await m.promptPassword("Desbloqueo Requerido","Este documento está cifrado. Ingrese su Frase Maestra para extraer los datos reales:");if(!u){i.className=o;return}if(await _.setMasterKey(u),n=await b.getOne("doc",s),n.decryptionError){m.toast("⛔ Frase incorrecta. No se puede extraer el documento.","danger"),i.className=o;return}}const l=await b.getOne("tpl",n.templateId),p=JSON.stringify({_INFO:"Extracto generado para depuración con IA",document:n,template:l},null,2);await navigator.clipboard.writeText(p),i.className="fa-solid fa-check text-success",m.toast("JSON copiado al portapapeles","success"),setTimeout(()=>{i.className=o},1500)}catch(n){console.error("Error al extraer JSON:",n),i.className="fa-solid fa-xmark text-danger",m.toast("Error al extraer el JSON de desarrollo.","danger"),setTimeout(()=>{i.className=o},1500)}}})},show(){this.container&&(this.container.style.display="block"),this.render()},hide(){this.container&&(this.container.style.display="none")}},j={getCurrentUser(){return R.currentUser},onAuthStateChanged(t){return ye(R,t)},async login(t,e){try{const s=(await ve(R,t,e)).user;if(!s.emailVerified)throw await K(R),new Error("Debes verificar tu correo electrónico antes de entrar.");return s}catch(a){throw console.error("[AUTH] Error:",a.code),a.message.includes("verificar tu correo")?a:new Error("Credenciales inválidas o error de conexión.")}},async signup(t,e){try{const s=(await fe(R,t,e)).user;return await be(s),await Ae(Re(Se,"users",s.uid),{email:t,createdAt:new Date().toISOString(),emailVerified:!1}),s}catch(a){throw new Error("Error en registro: "+a.message)}},async logout(){return _.clearSession(),K(R)}},Z={async getProfile(t){if(!t)return null;try{return await x.getRaw("user_profiles",t)}catch(e){return console.error("Error obteniendo el perfil:",e),null}},async saveProfile(t,e){if(!t)throw new Error("Usuario no autenticado");try{await x.set("user_profiles",t,{...e,updatedAt:new Date().toISOString()},{merge:!0})}catch(a){throw console.error("Error guardando el perfil:",a),a}}},pe=(t,e,a)=>{const s=t[e];return s?typeof s=="function"?s():Promise.resolve(s):new Promise((i,o)=>{(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(o.bind(null,new Error("Unknown variable dynamic import: "+e+(e.split("/").length!==a?". Note that variables only represent file names one level deep.":""))))})},q={_loadedRoles:new Set,_loadedSingles:new Set,_viewRoles:{designer:["setting","editor"],documents:["editor","viewer"],print:["printer"],visor:["printer"]},async loadRole(t){if(this._loadedRoles.has(t))return;const e=Object.keys(H),a=e.map(s=>{const i=`${t}-${s}`;if(!customElements.get(i))return pe(Object.assign({"../components/api/editor-api.js":()=>r(()=>import("./editor-api.BiQMR7Fx.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),"../components/api/printer-api.js":()=>r(()=>import("./printer-api.dkXNrwjZ.js"),__vite__mapDeps([4,1,2,3]),import.meta.url),"../components/api/setting-api.js":()=>r(()=>import("./setting-api.slqScpqW.js"),__vite__mapDeps([5,1,2,3]),import.meta.url),"../components/api/viewer-api.js":()=>r(()=>import("./viewer-api.DngR2Uff.js"),__vite__mapDeps([6,1,2,3]),import.meta.url),"../components/audio/editor-audio.js":()=>r(()=>import("./editor-audio.BcI6L78Z.js"),__vite__mapDeps([7,1,2,3]),import.meta.url),"../components/audio/printer-audio.js":()=>r(()=>import("./printer-audio.DY0makS1.js"),__vite__mapDeps([8,1,2,3]),import.meta.url),"../components/audio/setting-audio.js":()=>r(()=>import("./setting-audio.pUA9GcQp.js"),__vite__mapDeps([9,10,1,2,3]),import.meta.url),"../components/audio/viewer-audio.js":()=>r(()=>import("./viewer-audio.CsWIlbzx.js"),__vite__mapDeps([11,1,2,3]),import.meta.url),"../components/boolean/editor-boolean.js":()=>r(()=>import("./editor-boolean.rLihPtqd.js"),__vite__mapDeps([12,1,2,3]),import.meta.url),"../components/boolean/printer-boolean.js":()=>r(()=>import("./printer-boolean.DWHs2K6N.js"),__vite__mapDeps([13,1,2,3]),import.meta.url),"../components/boolean/setting-boolean.js":()=>r(()=>import("./setting-boolean.DkOLBy_r.js"),__vite__mapDeps([14,1,2,3]),import.meta.url),"../components/boolean/viewer-boolean.js":()=>r(()=>import("./viewer-boolean.Wq6ylvjG.js"),__vite__mapDeps([15,1,2,3]),import.meta.url),"../components/chart/editor-chart.js":()=>r(()=>import("./editor-chart.DUpCzqNY.js"),__vite__mapDeps([16,1,2,3,17]),import.meta.url),"../components/chart/printer-chart.js":()=>r(()=>import("./printer-chart.CBR4QWEy.js"),__vite__mapDeps([18,1,2,3,17]),import.meta.url),"../components/chart/setting-chart.js":()=>r(()=>import("./setting-chart.IyglW1S6.js"),__vite__mapDeps([19,1,2,3]),import.meta.url),"../components/chart/viewer-chart.js":()=>r(()=>import("./viewer-chart.z4bK7g23.js"),__vite__mapDeps([20,1,2,3,17]),import.meta.url),"../components/currency/editor-currency.js":()=>r(()=>import("./editor-currency.PwYJR4ry.js"),__vite__mapDeps([21,1,2,3]),import.meta.url),"../components/currency/printer-currency.js":()=>r(()=>import("./printer-currency.BqX1njaE.js"),__vite__mapDeps([22,1,2,3]),import.meta.url),"../components/currency/setting-currency.js":()=>r(()=>import("./setting-currency.kX65n2s6.js"),__vite__mapDeps([23,24,1,2,3]),import.meta.url),"../components/currency/viewer-currency.js":()=>r(()=>import("./viewer-currency.tB-Z9PF7.js"),__vite__mapDeps([25,1,2,3]),import.meta.url),"../components/date/editor-date.js":()=>r(()=>import("./editor-date.CJObLmed.js"),__vite__mapDeps([26,1,2,3]),import.meta.url),"../components/date/printer-date.js":()=>r(()=>import("./printer-date.BhZv-mtT.js"),__vite__mapDeps([27,1,2,3]),import.meta.url),"../components/date/setting-date.js":()=>r(()=>import("./setting-date.DnOTXXPg.js"),__vite__mapDeps([28,29,1,2,3]),import.meta.url),"../components/date/viewer-date.js":()=>r(()=>import("./viewer-date.D7vwoBfU.js"),__vite__mapDeps([30,1,2,3]),import.meta.url),"../components/datetime/editor-datetime.js":()=>r(()=>import("./editor-datetime.CS8EZEkc.js"),__vite__mapDeps([31,1,2,3]),import.meta.url),"../components/datetime/printer-datetime.js":()=>r(()=>import("./printer-datetime.BShbmZT-.js"),__vite__mapDeps([32,1,2,3]),import.meta.url),"../components/datetime/setting-datetime.js":()=>r(()=>import("./setting-datetime.CCgyRbe3.js"),__vite__mapDeps([29,1,2,3]),import.meta.url),"../components/datetime/viewer-datetime.js":()=>r(()=>import("./viewer-datetime.BE2HJLw8.js"),__vite__mapDeps([33,1,2,3]),import.meta.url),"../components/divider/editor-divider.js":()=>r(()=>import("./editor-divider.uzBvKYzn.js"),__vite__mapDeps([34,1,2,3]),import.meta.url),"../components/divider/printer-divider.js":()=>r(()=>import("./printer-divider.3agfhJ4F.js"),__vite__mapDeps([35,1,2,3]),import.meta.url),"../components/divider/setting-divider.js":()=>r(()=>import("./setting-divider.B4OX9E8k.js"),__vite__mapDeps([36,1,2,3]),import.meta.url),"../components/divider/viewer-divider.js":()=>r(()=>import("./viewer-divider.BRqA_x4t.js"),__vite__mapDeps([37,1,2,3]),import.meta.url),"../components/email/editor-email.js":()=>r(()=>import("./editor-email.BCuWtrCJ.js"),__vite__mapDeps([38,1,2,3]),import.meta.url),"../components/email/printer-email.js":()=>r(()=>import("./printer-email.omTH8QoT.js"),__vite__mapDeps([39,1,2,3]),import.meta.url),"../components/email/setting-email.js":()=>r(()=>import("./setting-email.CO3bAs9Y.js"),__vite__mapDeps([40,41,1,2,3]),import.meta.url),"../components/email/viewer-email.js":()=>r(()=>import("./viewer-email.DSmMvL65.js"),__vite__mapDeps([42,1,2,3]),import.meta.url),"../components/html/editor-html.js":()=>r(()=>import("./editor-html.DoGLq9h6.js"),__vite__mapDeps([43,1,2,3]),import.meta.url),"../components/html/printer-html.js":()=>r(()=>import("./printer-html.BfypK0XT.js"),__vite__mapDeps([44,1,2,3]),import.meta.url),"../components/html/setting-html.js":()=>r(()=>import("./setting-html.DIKN1aDU.js"),__vite__mapDeps([45,1,2,3]),import.meta.url),"../components/html/viewer-html.js":()=>r(()=>import("./viewer-html.CjjJII7p.js"),__vite__mapDeps([46,1,2,3]),import.meta.url),"../components/image/editor-image.js":()=>r(()=>import("./editor-image.CUjZ6_pS.js"),__vite__mapDeps([47,1,2,3]),import.meta.url),"../components/image/printer-image.js":()=>r(()=>import("./printer-image.Dris4rRr.js"),__vite__mapDeps([48,1,2,3]),import.meta.url),"../components/image/setting-image.js":()=>r(()=>import("./setting-image.CioHBqop.js"),__vite__mapDeps([49,10,1,2,3]),import.meta.url),"../components/image/viewer-image.js":()=>r(()=>import("./viewer-image.D7EaSHMP.js"),__vite__mapDeps([50,1,2,3]),import.meta.url),"../components/input/editor-input.js":()=>r(()=>import("./editor-input.Cvqz9-uB.js"),__vite__mapDeps([51,1,2,3]),import.meta.url),"../components/input/printer-input.js":()=>r(()=>import("./printer-input.D2IUkHqX.js"),__vite__mapDeps([52,1,2,3]),import.meta.url),"../components/input/setting-input.js":()=>r(()=>import("./setting-input.D2zfCgNz.js"),__vite__mapDeps([41,1,2,3]),import.meta.url),"../components/input/viewer-input.js":()=>r(()=>import("./viewer-input.CVepMjL5.js"),__vite__mapDeps([53,1,2,3]),import.meta.url),"../components/media/editor-media.js":()=>r(()=>import("./editor-media.BCX_nGGw.js"),__vite__mapDeps([54,1,2,3]),import.meta.url),"../components/media/printer-media.js":()=>r(()=>import("./printer-media.Cey6EvzG.js"),__vite__mapDeps([55,1,2,3]),import.meta.url),"../components/media/setting-media.js":()=>r(()=>import("./setting-media.C1ed5wTE.js"),__vite__mapDeps([10,1,2,3]),import.meta.url),"../components/media/viewer-media.js":()=>r(()=>import("./viewer-media.B6rwekJD.js"),__vite__mapDeps([56,1,2,3]),import.meta.url),"../components/number/editor-number.js":()=>r(()=>import("./editor-number.o-cXJjkf.js"),__vite__mapDeps([57,1,2,3]),import.meta.url),"../components/number/printer-number.js":()=>r(()=>import("./printer-number.CBc6iJYW.js"),__vite__mapDeps([58,1,2,3]),import.meta.url),"../components/number/setting-number.js":()=>r(()=>import("./setting-number.C_Ghihfm.js"),__vite__mapDeps([59,24,1,2,3]),import.meta.url),"../components/number/viewer-number.js":()=>r(()=>import("./viewer-number.Ku2aCoVl.js"),__vite__mapDeps([60,1,2,3]),import.meta.url),"../components/numeric/editor-numeric.js":()=>r(()=>import("./editor-numeric.D4RdEhvw.js"),__vite__mapDeps([61,1,2,3]),import.meta.url),"../components/numeric/printer-numeric.js":()=>r(()=>import("./printer-numeric.fjo3zpx_.js"),__vite__mapDeps([62,1,2,3]),import.meta.url),"../components/numeric/setting-numeric.js":()=>r(()=>import("./setting-numeric.DnfXpsZo.js"),__vite__mapDeps([24,1,2,3]),import.meta.url),"../components/numeric/viewer-numeric.js":()=>r(()=>import("./viewer-numeric.CQEqao_d.js"),__vite__mapDeps([63,1,2,3]),import.meta.url),"../components/password/editor-password.js":()=>r(()=>import("./editor-password.DY08ipuj.js"),__vite__mapDeps([64,1,2,3]),import.meta.url),"../components/password/printer-password.js":()=>r(()=>import("./printer-password.5GB6O0Rb.js"),__vite__mapDeps([65,1,2,3]),import.meta.url),"../components/password/setting-password.js":()=>r(()=>import("./setting-password.BnUqOMcs.js"),__vite__mapDeps([66,41,1,2,3]),import.meta.url),"../components/password/viewer-password.js":()=>r(()=>import("./viewer-password.BKtf-DCr.js"),__vite__mapDeps([67,1,2,3]),import.meta.url),"../components/percentage/editor-percentage.js":()=>r(()=>import("./editor-percentage.SCTrIDcN.js"),__vite__mapDeps([68,1,2,3]),import.meta.url),"../components/percentage/printer-percentage.js":()=>r(()=>import("./printer-percentage.CVPpcHSw.js"),__vite__mapDeps([69,1,2,3]),import.meta.url),"../components/percentage/setting-percentage.js":()=>r(()=>import("./setting-percentage.BaPPkDU-.js"),__vite__mapDeps([70,24,1,2,3]),import.meta.url),"../components/percentage/viewer-percentage.js":()=>r(()=>import("./viewer-percentage.knnAz5O-.js"),__vite__mapDeps([71,1,2,3]),import.meta.url),"../components/script/editor-script.js":()=>r(()=>import("./editor-script.Dk0DjPtA.js"),__vite__mapDeps([72,1,2,3,73]),import.meta.url),"../components/script/printer-script.js":()=>r(()=>import("./printer-script.DhkWeoxj.js"),__vite__mapDeps([74,1,2,3]),import.meta.url),"../components/script/setting-script.js":()=>r(()=>import("./setting-script.BANOqncP.js"),__vite__mapDeps([75,1,2,3,73]),import.meta.url),"../components/script/viewer-script.js":()=>r(()=>import("./viewer-script.CsuAu3Su.js"),__vite__mapDeps([76,1,2,3]),import.meta.url),"../components/section/editor-section.js":()=>r(()=>import("./editor-section.Tx5c5_CO.js"),__vite__mapDeps([77,1,2,3]),import.meta.url),"../components/section/printer-section.js":()=>r(()=>import("./printer-section.DQm4iCEm.js"),__vite__mapDeps([78,1,2,3]),import.meta.url),"../components/section/setting-section.js":()=>r(()=>import("./setting-section.CRY7BEYU.js"),__vite__mapDeps([79,36,1,2,3]),import.meta.url),"../components/section/viewer-section.js":()=>r(()=>import("./viewer-section.Dt-K_n3m.js"),__vite__mapDeps([80,1,2,3]),import.meta.url),"../components/select/editor-select.js":()=>r(()=>import("./editor-select.DEJ9XRpd.js"),__vite__mapDeps([81,1,2,3]),import.meta.url),"../components/select/printer-select.js":()=>r(()=>import("./printer-select.C9lmPiyz.js"),__vite__mapDeps([82,1,2,3]),import.meta.url),"../components/select/setting-select.js":()=>r(()=>import("./setting-select.Bru_cUIh.js"),__vite__mapDeps([83,1,2,3]),import.meta.url),"../components/select/viewer-select.js":()=>r(()=>import("./viewer-select.Vj8g8_j5.js"),__vite__mapDeps([84,1,2,3]),import.meta.url),"../components/string/editor-string.js":()=>r(()=>import("./editor-string.BIL15mvg.js"),__vite__mapDeps([85,1,2,3]),import.meta.url),"../components/string/printer-string.js":()=>r(()=>import("./printer-string.DUdi4nSO.js"),__vite__mapDeps([86,1,2,3]),import.meta.url),"../components/string/setting-string.js":()=>r(()=>import("./setting-string.Diwo7DP5.js"),__vite__mapDeps([87,41,1,2,3]),import.meta.url),"../components/string/viewer-string.js":()=>r(()=>import("./viewer-string.CaNn-lUw.js"),__vite__mapDeps([88,1,2,3]),import.meta.url),"../components/subsection/editor-subsection.js":()=>r(()=>import("./editor-subsection.mbar37gs.js"),__vite__mapDeps([89,1,2,3]),import.meta.url),"../components/subsection/printer-subsection.js":()=>r(()=>import("./printer-subsection.Bi3wRu27.js"),__vite__mapDeps([90,1,2,3]),import.meta.url),"../components/subsection/setting-subsection.js":()=>r(()=>import("./setting-subsection.C5Cd2w2Q.js"),__vite__mapDeps([91,36,1,2,3]),import.meta.url),"../components/subsection/viewer-subsection.js":()=>r(()=>import("./viewer-subsection.BsWmQ0IA.js"),__vite__mapDeps([92,1,2,3]),import.meta.url),"../components/table/editor-table.js":()=>r(()=>import("./editor-table.DVk0_OWU.js"),__vite__mapDeps([93,1,2,3,94,73]),import.meta.url),"../components/table/printer-table.js":()=>r(()=>import("./printer-table.B2UrCtcw.js"),__vite__mapDeps([95,1,2,3,94]),import.meta.url),"../components/table/setting-table.js":()=>r(()=>import("./setting-table.C8ZRHypl.js"),__vite__mapDeps([96,1,2,3,73]),import.meta.url),"../components/table/viewer-table.js":()=>r(()=>import("./viewer-table.Dqf7xeh4.js"),__vite__mapDeps([97,1,2,3,94]),import.meta.url),"../components/text/editor-text.js":()=>r(()=>import("./editor-text.ChQa-hSb.js"),__vite__mapDeps([98,1,2,3]),import.meta.url),"../components/text/printer-text.js":()=>r(()=>import("./printer-text.D70Wjo7t.js"),__vite__mapDeps([99,1,2,3]),import.meta.url),"../components/text/setting-text.js":()=>r(()=>import("./setting-text.Br1TMrYX.js"),__vite__mapDeps([100,41,1,2,3]),import.meta.url),"../components/text/viewer-text.js":()=>r(()=>import("./viewer-text.C7e67H2-.js"),__vite__mapDeps([101,1,2,3]),import.meta.url),"../components/time/editor-time.js":()=>r(()=>import("./editor-time.CTI_2TIO.js"),__vite__mapDeps([102,1,2,3]),import.meta.url),"../components/time/printer-time.js":()=>r(()=>import("./printer-time.YfQg0wWq.js"),__vite__mapDeps([103,1,2,3]),import.meta.url),"../components/time/setting-time.js":()=>r(()=>import("./setting-time.DDfT01Mw.js"),__vite__mapDeps([104,29,1,2,3]),import.meta.url),"../components/time/viewer-time.js":()=>r(()=>import("./viewer-time.C_RUWJvJ.js"),__vite__mapDeps([105,1,2,3]),import.meta.url),"../components/url/editor-url.js":()=>r(()=>import("./editor-url.CqNPUSpE.js"),__vite__mapDeps([106,1,2,3,73]),import.meta.url),"../components/url/printer-url.js":()=>r(()=>import("./printer-url.ur_yVkgu.js"),__vite__mapDeps([107,1,2,3]),import.meta.url),"../components/url/setting-url.js":()=>r(()=>import("./setting-url.Bvmq0jEN.js"),__vite__mapDeps([108,1,2,3]),import.meta.url),"../components/url/viewer-url.js":()=>r(()=>import("./viewer-url.CeyBBx-i.js"),__vite__mapDeps([109,1,2,3,73]),import.meta.url),"../components/video/editor-video.js":()=>r(()=>import("./editor-video.AFsEVz2t.js"),__vite__mapDeps([110,1,2,3]),import.meta.url),"../components/video/printer-video.js":()=>r(()=>import("./printer-video.B6h6Awk0.js"),__vite__mapDeps([111,1,2,3]),import.meta.url),"../components/video/setting-video.js":()=>r(()=>import("./setting-video.BOquED1L.js"),__vite__mapDeps([112,10,1,2,3]),import.meta.url),"../components/video/viewer-video.js":()=>r(()=>import("./viewer-video.El_w0S_X.js"),__vite__mapDeps([113,1,2,3]),import.meta.url)}),`../components/${s}/${t}-${s}.js`,4).catch(o=>{console.warn(`[ComponentLoader] No se pudo cargar ${t}-${s}:`,o.message)})});await Promise.all(a),this._loadedRoles.add(t),`${t}${e.length}`},async loadForView(t){const e=this._viewRoles[t]||[];e.length!==0&&(await Promise.all(e.map(a=>this.loadRole(a))),`${t}${e.join(", ")}`)},async loadInitial(t="documents"){`${t}`,await this.loadForView(t)},async loadSingle(t,e){const a=`${t}-${e}`;if(!this._loadedSingles.has(a)){if(customElements.get(a)){this._loadedSingles.add(a);return}try{await pe(Object.assign({"../components/api/editor-api.js":()=>r(()=>import("./editor-api.BiQMR7Fx.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),"../components/api/printer-api.js":()=>r(()=>import("./printer-api.dkXNrwjZ.js"),__vite__mapDeps([4,1,2,3]),import.meta.url),"../components/api/setting-api.js":()=>r(()=>import("./setting-api.slqScpqW.js"),__vite__mapDeps([5,1,2,3]),import.meta.url),"../components/api/viewer-api.js":()=>r(()=>import("./viewer-api.DngR2Uff.js"),__vite__mapDeps([6,1,2,3]),import.meta.url),"../components/audio/editor-audio.js":()=>r(()=>import("./editor-audio.BcI6L78Z.js"),__vite__mapDeps([7,1,2,3]),import.meta.url),"../components/audio/printer-audio.js":()=>r(()=>import("./printer-audio.DY0makS1.js"),__vite__mapDeps([8,1,2,3]),import.meta.url),"../components/audio/setting-audio.js":()=>r(()=>import("./setting-audio.pUA9GcQp.js"),__vite__mapDeps([9,10,1,2,3]),import.meta.url),"../components/audio/viewer-audio.js":()=>r(()=>import("./viewer-audio.CsWIlbzx.js"),__vite__mapDeps([11,1,2,3]),import.meta.url),"../components/boolean/editor-boolean.js":()=>r(()=>import("./editor-boolean.rLihPtqd.js"),__vite__mapDeps([12,1,2,3]),import.meta.url),"../components/boolean/printer-boolean.js":()=>r(()=>import("./printer-boolean.DWHs2K6N.js"),__vite__mapDeps([13,1,2,3]),import.meta.url),"../components/boolean/setting-boolean.js":()=>r(()=>import("./setting-boolean.DkOLBy_r.js"),__vite__mapDeps([14,1,2,3]),import.meta.url),"../components/boolean/viewer-boolean.js":()=>r(()=>import("./viewer-boolean.Wq6ylvjG.js"),__vite__mapDeps([15,1,2,3]),import.meta.url),"../components/chart/editor-chart.js":()=>r(()=>import("./editor-chart.DUpCzqNY.js"),__vite__mapDeps([16,1,2,3,17]),import.meta.url),"../components/chart/printer-chart.js":()=>r(()=>import("./printer-chart.CBR4QWEy.js"),__vite__mapDeps([18,1,2,3,17]),import.meta.url),"../components/chart/setting-chart.js":()=>r(()=>import("./setting-chart.IyglW1S6.js"),__vite__mapDeps([19,1,2,3]),import.meta.url),"../components/chart/viewer-chart.js":()=>r(()=>import("./viewer-chart.z4bK7g23.js"),__vite__mapDeps([20,1,2,3,17]),import.meta.url),"../components/currency/editor-currency.js":()=>r(()=>import("./editor-currency.PwYJR4ry.js"),__vite__mapDeps([21,1,2,3]),import.meta.url),"../components/currency/printer-currency.js":()=>r(()=>import("./printer-currency.BqX1njaE.js"),__vite__mapDeps([22,1,2,3]),import.meta.url),"../components/currency/setting-currency.js":()=>r(()=>import("./setting-currency.kX65n2s6.js"),__vite__mapDeps([23,24,1,2,3]),import.meta.url),"../components/currency/viewer-currency.js":()=>r(()=>import("./viewer-currency.tB-Z9PF7.js"),__vite__mapDeps([25,1,2,3]),import.meta.url),"../components/date/editor-date.js":()=>r(()=>import("./editor-date.CJObLmed.js"),__vite__mapDeps([26,1,2,3]),import.meta.url),"../components/date/printer-date.js":()=>r(()=>import("./printer-date.BhZv-mtT.js"),__vite__mapDeps([27,1,2,3]),import.meta.url),"../components/date/setting-date.js":()=>r(()=>import("./setting-date.DnOTXXPg.js"),__vite__mapDeps([28,29,1,2,3]),import.meta.url),"../components/date/viewer-date.js":()=>r(()=>import("./viewer-date.D7vwoBfU.js"),__vite__mapDeps([30,1,2,3]),import.meta.url),"../components/datetime/editor-datetime.js":()=>r(()=>import("./editor-datetime.CS8EZEkc.js"),__vite__mapDeps([31,1,2,3]),import.meta.url),"../components/datetime/printer-datetime.js":()=>r(()=>import("./printer-datetime.BShbmZT-.js"),__vite__mapDeps([32,1,2,3]),import.meta.url),"../components/datetime/setting-datetime.js":()=>r(()=>import("./setting-datetime.CCgyRbe3.js"),__vite__mapDeps([29,1,2,3]),import.meta.url),"../components/datetime/viewer-datetime.js":()=>r(()=>import("./viewer-datetime.BE2HJLw8.js"),__vite__mapDeps([33,1,2,3]),import.meta.url),"../components/divider/editor-divider.js":()=>r(()=>import("./editor-divider.uzBvKYzn.js"),__vite__mapDeps([34,1,2,3]),import.meta.url),"../components/divider/printer-divider.js":()=>r(()=>import("./printer-divider.3agfhJ4F.js"),__vite__mapDeps([35,1,2,3]),import.meta.url),"../components/divider/setting-divider.js":()=>r(()=>import("./setting-divider.B4OX9E8k.js"),__vite__mapDeps([36,1,2,3]),import.meta.url),"../components/divider/viewer-divider.js":()=>r(()=>import("./viewer-divider.BRqA_x4t.js"),__vite__mapDeps([37,1,2,3]),import.meta.url),"../components/email/editor-email.js":()=>r(()=>import("./editor-email.BCuWtrCJ.js"),__vite__mapDeps([38,1,2,3]),import.meta.url),"../components/email/printer-email.js":()=>r(()=>import("./printer-email.omTH8QoT.js"),__vite__mapDeps([39,1,2,3]),import.meta.url),"../components/email/setting-email.js":()=>r(()=>import("./setting-email.CO3bAs9Y.js"),__vite__mapDeps([40,41,1,2,3]),import.meta.url),"../components/email/viewer-email.js":()=>r(()=>import("./viewer-email.DSmMvL65.js"),__vite__mapDeps([42,1,2,3]),import.meta.url),"../components/html/editor-html.js":()=>r(()=>import("./editor-html.DoGLq9h6.js"),__vite__mapDeps([43,1,2,3]),import.meta.url),"../components/html/printer-html.js":()=>r(()=>import("./printer-html.BfypK0XT.js"),__vite__mapDeps([44,1,2,3]),import.meta.url),"../components/html/setting-html.js":()=>r(()=>import("./setting-html.DIKN1aDU.js"),__vite__mapDeps([45,1,2,3]),import.meta.url),"../components/html/viewer-html.js":()=>r(()=>import("./viewer-html.CjjJII7p.js"),__vite__mapDeps([46,1,2,3]),import.meta.url),"../components/image/editor-image.js":()=>r(()=>import("./editor-image.CUjZ6_pS.js"),__vite__mapDeps([47,1,2,3]),import.meta.url),"../components/image/printer-image.js":()=>r(()=>import("./printer-image.Dris4rRr.js"),__vite__mapDeps([48,1,2,3]),import.meta.url),"../components/image/setting-image.js":()=>r(()=>import("./setting-image.CioHBqop.js"),__vite__mapDeps([49,10,1,2,3]),import.meta.url),"../components/image/viewer-image.js":()=>r(()=>import("./viewer-image.D7EaSHMP.js"),__vite__mapDeps([50,1,2,3]),import.meta.url),"../components/input/editor-input.js":()=>r(()=>import("./editor-input.Cvqz9-uB.js"),__vite__mapDeps([51,1,2,3]),import.meta.url),"../components/input/printer-input.js":()=>r(()=>import("./printer-input.D2IUkHqX.js"),__vite__mapDeps([52,1,2,3]),import.meta.url),"../components/input/setting-input.js":()=>r(()=>import("./setting-input.D2zfCgNz.js"),__vite__mapDeps([41,1,2,3]),import.meta.url),"../components/input/viewer-input.js":()=>r(()=>import("./viewer-input.CVepMjL5.js"),__vite__mapDeps([53,1,2,3]),import.meta.url),"../components/media/editor-media.js":()=>r(()=>import("./editor-media.BCX_nGGw.js"),__vite__mapDeps([54,1,2,3]),import.meta.url),"../components/media/printer-media.js":()=>r(()=>import("./printer-media.Cey6EvzG.js"),__vite__mapDeps([55,1,2,3]),import.meta.url),"../components/media/setting-media.js":()=>r(()=>import("./setting-media.C1ed5wTE.js"),__vite__mapDeps([10,1,2,3]),import.meta.url),"../components/media/viewer-media.js":()=>r(()=>import("./viewer-media.B6rwekJD.js"),__vite__mapDeps([56,1,2,3]),import.meta.url),"../components/number/editor-number.js":()=>r(()=>import("./editor-number.o-cXJjkf.js"),__vite__mapDeps([57,1,2,3]),import.meta.url),"../components/number/printer-number.js":()=>r(()=>import("./printer-number.CBc6iJYW.js"),__vite__mapDeps([58,1,2,3]),import.meta.url),"../components/number/setting-number.js":()=>r(()=>import("./setting-number.C_Ghihfm.js"),__vite__mapDeps([59,24,1,2,3]),import.meta.url),"../components/number/viewer-number.js":()=>r(()=>import("./viewer-number.Ku2aCoVl.js"),__vite__mapDeps([60,1,2,3]),import.meta.url),"../components/numeric/editor-numeric.js":()=>r(()=>import("./editor-numeric.D4RdEhvw.js"),__vite__mapDeps([61,1,2,3]),import.meta.url),"../components/numeric/printer-numeric.js":()=>r(()=>import("./printer-numeric.fjo3zpx_.js"),__vite__mapDeps([62,1,2,3]),import.meta.url),"../components/numeric/setting-numeric.js":()=>r(()=>import("./setting-numeric.DnfXpsZo.js"),__vite__mapDeps([24,1,2,3]),import.meta.url),"../components/numeric/viewer-numeric.js":()=>r(()=>import("./viewer-numeric.CQEqao_d.js"),__vite__mapDeps([63,1,2,3]),import.meta.url),"../components/password/editor-password.js":()=>r(()=>import("./editor-password.DY08ipuj.js"),__vite__mapDeps([64,1,2,3]),import.meta.url),"../components/password/printer-password.js":()=>r(()=>import("./printer-password.5GB6O0Rb.js"),__vite__mapDeps([65,1,2,3]),import.meta.url),"../components/password/setting-password.js":()=>r(()=>import("./setting-password.BnUqOMcs.js"),__vite__mapDeps([66,41,1,2,3]),import.meta.url),"../components/password/viewer-password.js":()=>r(()=>import("./viewer-password.BKtf-DCr.js"),__vite__mapDeps([67,1,2,3]),import.meta.url),"../components/percentage/editor-percentage.js":()=>r(()=>import("./editor-percentage.SCTrIDcN.js"),__vite__mapDeps([68,1,2,3]),import.meta.url),"../components/percentage/printer-percentage.js":()=>r(()=>import("./printer-percentage.CVPpcHSw.js"),__vite__mapDeps([69,1,2,3]),import.meta.url),"../components/percentage/setting-percentage.js":()=>r(()=>import("./setting-percentage.BaPPkDU-.js"),__vite__mapDeps([70,24,1,2,3]),import.meta.url),"../components/percentage/viewer-percentage.js":()=>r(()=>import("./viewer-percentage.knnAz5O-.js"),__vite__mapDeps([71,1,2,3]),import.meta.url),"../components/script/editor-script.js":()=>r(()=>import("./editor-script.Dk0DjPtA.js"),__vite__mapDeps([72,1,2,3,73]),import.meta.url),"../components/script/printer-script.js":()=>r(()=>import("./printer-script.DhkWeoxj.js"),__vite__mapDeps([74,1,2,3]),import.meta.url),"../components/script/setting-script.js":()=>r(()=>import("./setting-script.BANOqncP.js"),__vite__mapDeps([75,1,2,3,73]),import.meta.url),"../components/script/viewer-script.js":()=>r(()=>import("./viewer-script.CsuAu3Su.js"),__vite__mapDeps([76,1,2,3]),import.meta.url),"../components/section/editor-section.js":()=>r(()=>import("./editor-section.Tx5c5_CO.js"),__vite__mapDeps([77,1,2,3]),import.meta.url),"../components/section/printer-section.js":()=>r(()=>import("./printer-section.DQm4iCEm.js"),__vite__mapDeps([78,1,2,3]),import.meta.url),"../components/section/setting-section.js":()=>r(()=>import("./setting-section.CRY7BEYU.js"),__vite__mapDeps([79,36,1,2,3]),import.meta.url),"../components/section/viewer-section.js":()=>r(()=>import("./viewer-section.Dt-K_n3m.js"),__vite__mapDeps([80,1,2,3]),import.meta.url),"../components/select/editor-select.js":()=>r(()=>import("./editor-select.DEJ9XRpd.js"),__vite__mapDeps([81,1,2,3]),import.meta.url),"../components/select/printer-select.js":()=>r(()=>import("./printer-select.C9lmPiyz.js"),__vite__mapDeps([82,1,2,3]),import.meta.url),"../components/select/setting-select.js":()=>r(()=>import("./setting-select.Bru_cUIh.js"),__vite__mapDeps([83,1,2,3]),import.meta.url),"../components/select/viewer-select.js":()=>r(()=>import("./viewer-select.Vj8g8_j5.js"),__vite__mapDeps([84,1,2,3]),import.meta.url),"../components/string/editor-string.js":()=>r(()=>import("./editor-string.BIL15mvg.js"),__vite__mapDeps([85,1,2,3]),import.meta.url),"../components/string/printer-string.js":()=>r(()=>import("./printer-string.DUdi4nSO.js"),__vite__mapDeps([86,1,2,3]),import.meta.url),"../components/string/setting-string.js":()=>r(()=>import("./setting-string.Diwo7DP5.js"),__vite__mapDeps([87,41,1,2,3]),import.meta.url),"../components/string/viewer-string.js":()=>r(()=>import("./viewer-string.CaNn-lUw.js"),__vite__mapDeps([88,1,2,3]),import.meta.url),"../components/subsection/editor-subsection.js":()=>r(()=>import("./editor-subsection.mbar37gs.js"),__vite__mapDeps([89,1,2,3]),import.meta.url),"../components/subsection/printer-subsection.js":()=>r(()=>import("./printer-subsection.Bi3wRu27.js"),__vite__mapDeps([90,1,2,3]),import.meta.url),"../components/subsection/setting-subsection.js":()=>r(()=>import("./setting-subsection.C5Cd2w2Q.js"),__vite__mapDeps([91,36,1,2,3]),import.meta.url),"../components/subsection/viewer-subsection.js":()=>r(()=>import("./viewer-subsection.BsWmQ0IA.js"),__vite__mapDeps([92,1,2,3]),import.meta.url),"../components/table/editor-table.js":()=>r(()=>import("./editor-table.DVk0_OWU.js"),__vite__mapDeps([93,1,2,3,94,73]),import.meta.url),"../components/table/printer-table.js":()=>r(()=>import("./printer-table.B2UrCtcw.js"),__vite__mapDeps([95,1,2,3,94]),import.meta.url),"../components/table/setting-table.js":()=>r(()=>import("./setting-table.C8ZRHypl.js"),__vite__mapDeps([96,1,2,3,73]),import.meta.url),"../components/table/viewer-table.js":()=>r(()=>import("./viewer-table.Dqf7xeh4.js"),__vite__mapDeps([97,1,2,3,94]),import.meta.url),"../components/text/editor-text.js":()=>r(()=>import("./editor-text.ChQa-hSb.js"),__vite__mapDeps([98,1,2,3]),import.meta.url),"../components/text/printer-text.js":()=>r(()=>import("./printer-text.D70Wjo7t.js"),__vite__mapDeps([99,1,2,3]),import.meta.url),"../components/text/setting-text.js":()=>r(()=>import("./setting-text.Br1TMrYX.js"),__vite__mapDeps([100,41,1,2,3]),import.meta.url),"../components/text/viewer-text.js":()=>r(()=>import("./viewer-text.C7e67H2-.js"),__vite__mapDeps([101,1,2,3]),import.meta.url),"../components/time/editor-time.js":()=>r(()=>import("./editor-time.CTI_2TIO.js"),__vite__mapDeps([102,1,2,3]),import.meta.url),"../components/time/printer-time.js":()=>r(()=>import("./printer-time.YfQg0wWq.js"),__vite__mapDeps([103,1,2,3]),import.meta.url),"../components/time/setting-time.js":()=>r(()=>import("./setting-time.DDfT01Mw.js"),__vite__mapDeps([104,29,1,2,3]),import.meta.url),"../components/time/viewer-time.js":()=>r(()=>import("./viewer-time.C_RUWJvJ.js"),__vite__mapDeps([105,1,2,3]),import.meta.url),"../components/url/editor-url.js":()=>r(()=>import("./editor-url.CqNPUSpE.js"),__vite__mapDeps([106,1,2,3,73]),import.meta.url),"../components/url/printer-url.js":()=>r(()=>import("./printer-url.ur_yVkgu.js"),__vite__mapDeps([107,1,2,3]),import.meta.url),"../components/url/setting-url.js":()=>r(()=>import("./setting-url.Bvmq0jEN.js"),__vite__mapDeps([108,1,2,3]),import.meta.url),"../components/url/viewer-url.js":()=>r(()=>import("./viewer-url.CeyBBx-i.js"),__vite__mapDeps([109,1,2,3,73]),import.meta.url),"../components/video/editor-video.js":()=>r(()=>import("./editor-video.AFsEVz2t.js"),__vite__mapDeps([110,1,2,3]),import.meta.url),"../components/video/printer-video.js":()=>r(()=>import("./printer-video.B6h6Awk0.js"),__vite__mapDeps([111,1,2,3]),import.meta.url),"../components/video/setting-video.js":()=>r(()=>import("./setting-video.BOquED1L.js"),__vite__mapDeps([112,10,1,2,3]),import.meta.url),"../components/video/viewer-video.js":()=>r(()=>import("./viewer-video.El_w0S_X.js"),__vite__mapDeps([113,1,2,3]),import.meta.url)}),`../components/${e}/${t}-${e}.js`,4),this._loadedSingles.add(a)}catch(s){console.warn(`[ComponentLoader] No se pudo cargar ${t}-${e}:`,s.message)}}},async loadBatch(t,e){const a=e.map(s=>this.loadSingle(t,s));await Promise.all(a)},get loadedRoles(){return[...this._loadedRoles]},get loadedSingles(){return[...this._loadedSingles]}},Y={async print(t,e){await q.loadRole("printer");let a=await b.getOne("doc",t);if(!a){m.toast("Documento no encontrado.","danger");return}if(a.isEncrypted&&a.decryptionError){m.toast("El documento está bloqueado. Desbloquéelo en el visor primero.","danger");return}const s=await b.getOne("tpl",a.templateId);if(!s){m.toast("La plantilla base no se encuentra.","danger");return}const i=j.getCurrentUser(),o=i?await Z.getProfile(i.uid):null;let n=document.getElementById("print-iframe");n||(n=document.createElement("iframe"),n.id="print-iframe",n.style.position="fixed",n.style.left="-10000px",n.style.top="0",n.style.width="100%",n.style.height="100%",document.body.appendChild(n));const l=n.contentWindow.document,d=new Date(a.date).toLocaleDateString("es-ES",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"});let p="";for(let h=1;h<=12;h++)p+=`.col-${h} { width: ${h/12*100}%; } `;let u="";if(o&&o.name&&o.name.trim()!==""){const h=o.logoBase64?`<img src="${o.logoBase64}" style="max-height: 60px; display: block; margin-bottom: 5px;">`:"",c=`<strong style="font-size: 0.9rem; color: #212529; text-transform: uppercase;">${o.name}</strong>`;u=`<div style="display: flex; flex-direction: column; align-items: flex-start; padding-right: 20px; margin-right: 20px; border-right: 2px solid #dee2e6;">${h}${c}</div>`}l.open(),l.write(`
        <html>
          <head>
            <title>${a.title}</title>
...
Ingrese su Frase Maestra para habilitar el editor:`);if(!s)return this.handlers.onBack();if(!await b.verifyMasterPhrase(s))return m.toast("⛔ Frase incorrecta. No tiene permiso para usar esta plantilla.","danger"),this.handlers.onBack()}this.currentDocId=null,this.currentTplId=t,this.titleInput.value="",this.titleInput.readOnly=!1;const a=document.getElementById("doc-template-selector");a&&(a.style.display="none"),await this.renderCanvas(t,[]),this.setupSaveButton("Guardar Documento")},async loadForEditing(t){const e=await b.getOne("doc",t);e&&(this.currentDocId=t,this.currentTplId=e.templateId,this.titleInput.value=e.title,this.titleInput.readOnly=!1,await this.renderCanvas(e.templateId,e.content),this.setupSaveButton("Guardar Cambios"))},async renderCanvas(t,e){const a=await b.getOne("tpl",t);if(!a)return;this.canvas.innerHTML="",a.elements.filter(o=>N(o)).length>0?(this.canvas.classList.add("has-sections","p-0","bg-transparent","shadow-none"),this._renderStructuredUI(a.elements,e)):(this.canvas.classList.remove("has-sections","p-0","bg-transparent","shadow-none"),a.elements.forEach(o=>{const n=this._createComponent(o,e);this.canvas.appendChild(n)}))},_renderStructuredUI(t,e){const a=[];let s={title:"Principal",def:null,elements:[]};t.forEach(d=>{N(d)?((s.elements.length>0||s.title!=="Principal")&&a.push(s),s={title:d.title||"Sección",def:d,elements:[]}):s.elements.push(d)}),a.push(s);const i=`nav-tabs-${this.currentTplId}`;let o=`<ul class="nav nav-tabs nav-tabs-custom d-none d-md-flex no-print mb-0" id="${i}" role="tablist">`,n=`<div class="tab-content tab-content-custom" id="${i}Content">`,l=-1;a.forEach((d,p)=>{const u=d.def;!(u&&(u.hideEdit===!0||u.hideEdit==="true"||u.hideEdit===""||u["hide-edit"]===!0||u["hide-edit"]==="true"||u["hide-edit"]===""))&&l===-1&&(l=p)}),l===-1&&(l=0),a.forEach((d,p)=>{const u=`sec-${p}-${this.currentTplId}`,h=p===l,c=d.def,v=c&&(c.hideEdit===!0||c.hideEdit==="true"||c.hideEdit===""||c["hide-edit"]===!0||c["hide-edit"]==="true"||c["hide-edit"]==="")?" d-none":"";o+=`
    ...
    `,this._bindEvents())},_bindEvents(){const t=document.getElementById("btn-export");t&&(t.onclick=()=>ge.openBackupSelector());const e=document.getElementById("import-file");e&&(e.onchange=a=>{a.target.files.length>0&&(ge.openRestoreSelector(a.target.files[0]),a.target.value="")})}},U={renderSetupModal(){document.body.insertAdjacentHTML("beforeend",`
