import{B as N,d as $}from"./BaseElement.VF2wA_rP.js";import{b as I,T,g as w,h as R,e as A,U as E,i as P}from"./Elements.B8W_hxTW.js";import{S as O}from"./main.DROJMqVA.js";import"./FormulaEngine.CYGke5JL.js";import"./preload-helper.bdfVo_kr.js";const M={getComponentsHierarchy(){const f=[],e=document.getElementById("designer-canvas");if(!e)return f;try{Array.from(e.querySelectorAll(".designer-item-wrapper")).forEach(n=>{const i=n.querySelector("#comp-content"),a=i?i.firstElementChild:null;if(a&&typeof a.getData=="function"){const o=a.getData();if(!o.id||I(o)||a.tagName.toLowerCase()===T.section||I(o)||a.tagName.toLowerCase()===T.subsection)return;const b=w(o.tag||a),p=R(o.tag||a,{icon:"🏷️",longLabel:"Componente",color:"secondary"}),g={id:o.id,title:o.title||o.text||"Sin título",icon:p.icon,typeKey:b,typeLabel:p.longLabel,color:p.color,children:[]};b==="table"&&Array.isArray(o.columns)&&o.columns.forEach(c=>{if(c.id){const d=c.tag?w(c.tag):"string",v=A[d]||{icon:"🏷️",longLabel:"Columna",color:"secondary"};g.children.push({id:c.id,title:c.title||"Sin título",icon:v.icon,typeKey:d,typeLabel:v.longLabel,color:v.color})}}),f.push(g)}})}catch(t){console.warn("Error al construir la jerarquía de componentes desde getData().",t)}return f}},h=$("script","scriptCode"),x=f=>{var e;return((e=A[f])==null?void 0:e.aiPrompt)||"No hay reglas específicas."};function D(){return`=== API DISPONIBLE DENTRO DE TU CÓDIGO (doc / row) ===
...
`,d=D(),r=!!((S=document.getElementById(`ai-attach-detail-${this.id}`))!=null&&S.checked)?`
...
"${e}"`}async _resyncAiClipboard(){if(!this._aiPromptCopied||!this._lastAiIdea)return;const e=this._buildAiSuperPromptText(this._lastAiIdea);await E.copyTextSmart(e,"Prompt del Script")!=="modal"&&window.DialogService&&window.DialogService.toast&&window.DialogService.toast("🔄 Prompt actualizado en el portapapeles (cambió la opción de archivo adjunto).","info")}_downloadAiReference(){E.downloadTextFile("doc-engine-referencia-scripting-api.txt",D()),window.DialogService&&window.DialogService.toast&&window.DialogService.toast("Referencia descargada.","success")}async _openServiceSelector(){var i;const e=`modal-selector-${this.id}`;(i=document.getElementById(e))==null||i.remove(),document.body.insertAdjacentHTML("beforeend",`
    ...
    `);const t=document.getElementById(e),n=new bootstrap.Modal(t);n.show(),t.addEventListener("hidden.bs.modal",()=>t.remove());try{const[a,o]=await Promise.all([O.getAll("doc"),O.getAll("tpl")]),b={};o.forEach(r=>b[r.id]=r);const p=[];a.forEach(r=>{const u=b[r.templateId];if(!u||!u.elements)return;const l=u.elements.filter(s=>P(s)&&s.esServicio);l.length>0&&p.push({doc:r,tplTitle:u.title,servicios:l})});const g=document.getElementById(`${e}-body`);if(p.length===0){g.innerHTML='<div class="alert alert-warning small mb-0">No se encontraron documentos que contengan scripts marcados como Servicio.</div>';return}g.innerHTML=`
        <div class="mb-3">
...
${c.stack||c.message}`}}}}customElements.define("setting-script",q);export{q as SettingScript};
